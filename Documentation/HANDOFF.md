...
**Foundation will not resolve `/tmp`, and the two APIs named for the job
both lie about it.** `URL.resolvingSymlinksInPath()` and
`NSString.resolvingSymlinksInPath` both return `/tmp/x` unchanged, because
Foundation deliberately standardises the `/private` prefix *away*; only
`realpath(3)` answers `/private/tmp/x`, and only for a path that exists.
This matters because a session's transcript is filed under its *resolved*
working directory — a session in `/tmp/x` lives under `-private-tmp-x` — so
deriving that folder from the reported path looks somewhere that does not
exist and the carry declines with "its transcript is missing", which is true
about the wrong place. Reaching for the obvious API produces a function that
looks right, passes a careless test, and resolves nothing.

**The transfer's security model settled on pinning the commit rather than
signing the brief, and that was a choice.** Item 5 proposed git SSH commit
signing so that a pushed branch could not pose as an instruction. What
shipped instead is a full sha decided by the launcher and passed to the
destination, because the question the destination has to answer is not *who
wrote this* but *is this the thing I was just sent* — the drag or the `move`
is the instruction, and it arrives over an authenticated connection. Signing
answers origin and says nothing about intent; it also would not have covered
`CLAUDE.md`, comments or fixtures, which a successor reads while working.
